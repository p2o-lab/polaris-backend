...
describe('AnaProcessValueInMockup', () => {
    ...
    describe('dynamic', () => {
        ...
        it('set and get VExt, >VSclMax', async () => {
            await connection.writeOpcUaNode('ns=1;s=Variable.VExt', namespaceUrl, 1.1, 'Double');
        ...
        it('set and get VExt', async () => {
            await connection.writeOpcUaNode('ns=1;s=Variable.VExt', namespaceUrl, 0, 'Double');
            await connection.readOpcUaNode('ns=1;s=Variable.VExt', namespaceUrl)
                .then(datavalue => expect(datavalue?.value.value).to.equal(0));
        }).timeout(3000);
        //TODO get the rest
